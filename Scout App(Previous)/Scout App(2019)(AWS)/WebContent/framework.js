...
function submit(){
	...
	const data = new Array(teamNum, matchNum, isValid, startingPos, startingLv, isCrossedLine, playStyle, isGroundHatch, 
						   ...
						   sFarRocketHatchNum, sFarRocketCargoNum, sCargoShipHatchNum, sCargoShipCargoNum, sCloseRocketHatchNum,
						   sCloseRocketCargoNum, sTotalHatch, sTotalCargo, tFarRocketHatchNum, tFarRocketCargoNum, tCargoShipHatchNum,
						   tCargoShipCargoNum, tCloseRocketHatchNum, tCloseRocketCargoNum, tTotalHatch, tTotalCargo, farRocketHatchNum,
						   farRocketCargoNum, cargoShipHatchNum, cargoShipCargoNum, closeRocketHatchNum, closeRocketCargoNum, totalHatch, totalCargo);
	var rawData = '';
	...
	if(navigator.onLine) {
		...
		for(var i=0; i<keys.length; i++) {
			passedData = localStorage.getItem(keys[i]);
	...
	/* 	To Do:
	 *	add 'name' to a list of comma delimited strings as a way to keep track of all the keys
	 *	add each entry into the localStorage map where 'name' maps to match data
	 *	
	 *	if an internet connection is available
	 *		grab list of keys from localStorage and use each key to generate a new .csv file
	 *		where the key is the name of the file.
	 *
	...
	if(!navigator.onLine) {
		if(teamNum !== "" && matchNum !==""){
			if(localStorage.getItem("data") !== null) {
				localStorage.setItem("data",(localStorage.getItem("data") + dataString));
			} else {
		...
		if(teamNum !== "" && matchNum !==""){
			...
			var passedData = new Array(localStorage.getItem("data"));
